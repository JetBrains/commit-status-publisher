...
public class GitlabPublisherTest extends HttpPublisherTest {
  ...
  public GitlabPublisherTest() {
    ...
    myExpectedRegExps.put(EventToTest.MERGED, String.format(".*/projects/owner%%2Fproject/statuses/%s.*ENTITY:.*pending.*%s by %s.*%s.*Link leads to the actual build.*", REVISION, DefaultStatusMessages.BUILD_REMOVED_FROM_QUEUE, USER.toLowerCase(), COMMENT));
  ...
  public void test_buildFinishedSuccessfully_server_url_with_subdir() throws Exception {
    ...
    myPublisher = new GitlabPublisher(myPublisherSettings, myBuildType, FEATURE_ID, myWebLinks, params, myProblems, myCustomDataStorageManager);
  ...
  protected void setUp() throws Exception {
    ...
    super.setUp();
    myPublisherSettings = new GitlabSettings(new MockPluginDescriptor(), myWebLinks, myProblems, myTrustStoreProvider, myCustomDataStorageManager);
    Map<String, String> params = getPublisherParams();
    myPublisher = new GitlabPublisher(myPublisherSettings, myBuildType, FEATURE_ID, myWebLinks, params, myProblems, myCustomDataStorageManager);

...
public class BitbucketServer74PublisherTest extends BaseStashPublisherTest {
  ...
  public BitbucketServer74PublisherTest() {
    ...
    myExpectedRegExps.put(EventToTest.REMOVED, String.format(".*projects/owner/repos/project/commits/%s.*ENTITY:.*%s by %s.*%s.*FAILED.*", REVISION, DefaultStatusMessages.BUILD_REMOVED_FROM_QUEUE, USER.toLowerCase(), COMMENT));
    myExpectedRegExps.put(EventToTest.MERGED, String.format(".*projects/owner/repos/project/commits/%s.*ENTITY:.*%s by %s.*%s.*INPROGRESS.*Link leads to the actual build", REVISION, DefaultStatusMessages.BUILD_REMOVED_FROM_QUEUE, USER.toLowerCase(), COMMENT));

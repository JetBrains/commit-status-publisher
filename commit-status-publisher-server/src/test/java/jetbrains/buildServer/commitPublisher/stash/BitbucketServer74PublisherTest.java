...
public class BitbucketServer74PublisherTest extends BaseStashPublisherTest {
  ...
  public BitbucketServer74PublisherTest() {
    ...
    myExpectedRegExps.put(EventToTest.REMOVED, String.format(".*projects/owner/repos/project/commits/%s.*ENTITY:.*%s by %s.*%s.*INPROGRESS.*", REVISION, DefaultStatusMessages.BUILD_REMOVED_FROM_QUEUE, USER.toLowerCase(), COMMENT));

...
class GitlabPublisher extends HttpBasedCommitStatusPublisher<GitlabBuildStatus> {
  ...
  public String getApiUrl(@Nullable String vcsRootUrl) throws PublisherException {
    if (!StringUtil.isEmptyOrSpaces(myParams.get(GitlabConstants.API_URL)))
      return HttpHelper.stripTrailingSlash(myParams.get(GitlabConstants.API_URL));
  ...
  private static boolean supportMergeResults(@NotNull BuildType buildType) {
    if (buildType instanceof InternalParameters) {
      return ((InternalParameters)buildType).getBooleanInternalParameterOrTrue(GitlabConstants.FEATURE_TOGGLE_MERGE_RESULTS);
    }

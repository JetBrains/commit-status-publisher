...
class GitlabPublisher extends HttpBasedCommitStatusPublisher {
  ...
  @Nullable
  private RevisionStatus getRevisionStatus(@NotNull BuildPromotion buildPromotion, @Nullable GitLabCommitStatus commitStatus) {
  ...
  private Event getTriggeredEvent(GitLabCommitStatus commitStatus) {
    ...
    if (status == null) {
      LOG.warn("Can not find GitLab status for received status \"" + commitStatus.status + "\". Related event can not be calculated");
      return null;
    }

    switch (status) {
      case SUCCESS:
        if (commitStatus.description != null && commitStatus.description.equals(DefaultStatusMessages.BUILD_MARKED_SUCCESSFULL)) {
          return Event.MARKED_AS_SUCCESSFUL;
        }
      ...
      case PENDING:
        return Event.QUEUED;
      default:
        LOG.warn("No event is assosiated with GitLab status \"" + status + "\". Related event can not be defined");
    }
    return null;
  }

...
import jetbrains.buildServer.commitPublisher.stash.data.StashBuildStatus;
...
class StashPublisher extends HttpBasedCommitStatusPublisher {
  ...
  private RevisionStatus getRevisionStatus(@NotNull BuildPromotion buildPromotion, @Nullable JsonStashBuildStatus buildStatus) {
    if (buildStatus == null) {
      return null;
    }
    Event event = getTriggeredEvent(buildStatus);
    boolean isSameBuild = isSameBuild(buildPromotion, buildStatus.url);
    ...
    switch (buildStatus.state) {
      ...
      case INPROGRESS:
        ...
        return buildStatus.description.contains(DefaultStatusMessages.BUILD_STARTED) ? Event.STARTED :
               buildStatus.description.contains(DefaultStatusMessages.BUILD_QUEUED) ? Event.QUEUED :
               buildStatus.description.contains(DefaultStatusMessages.BUILD_REMOVED_FROM_QUEUE) ? Event.REMOVED_FROM_QUEUE :
               buildStatus.description.contains(DefaultStatusMessages.BUILD_MARKED_SUCCESSFULL) ? Event.MARKED_AS_SUCCESSFUL : Event.STARTED;
      default:
        LOG.warn("No event is assosiated with BitBucket status \"" + buildStatus.state + "\". Related event can not be defined");
    }
    return null;
  }
  ...
  private abstract class BaseBitbucketEndpoint implements BitbucketEndpoint {
    ...
    private JsonStashBuildStatus convertToActualStatus(@Nullable DeprecatedJsonStashBuildStatuses.Status status) {
      ...
      JsonStashBuildStatus result = new JsonStashBuildStatus();

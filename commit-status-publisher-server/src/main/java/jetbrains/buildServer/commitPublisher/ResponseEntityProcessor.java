...
public class ResponseEntityProcessor<T> extends DefaultHttpResponseProcessor {
  ...
  public ResponseEntityProcessor(Class<T> type) {
    this(type, "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
  }

  public ResponseEntityProcessor(Class<T> type, String dateFormat) {
    myType = type;
    myGson = new GsonBuilder().setDateFormat(dateFormat).create();
  }
